...
import { renderDigitalSvg, renderAdjustableModernHtml, renderAdjustableDigitalHtml, calculateDurationAngles, calculateClockAngles, calculateStopwatchAngles } from './renderers.ts';
...
if (!displayModern || !btnTimerPrimary || !btnTimerReset || !btnTimerStop || !timerInput) {
	...
	store.subscribe((state) => {
		...
		if (mode === 'timer') {
			...
			// Modern
			if (modernTimeText) modernTimeText.innerHTML = renderAdjustableModernHtml(formatted, timer.status === 'running');
			...
			if (digitalSvgContainer && style === 'digital') {
				digitalSvgContainer.innerHTML = renderAdjustableDigitalHtml(formatted, timer.status === 'running');
			}
